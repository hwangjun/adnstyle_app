var UIController=( function() {
    ...
    function updateSideBarH() {
        ...
        if(getSideBarState()==false){
            if( Shared.ua.isMobile && ( Shared.ua.veriOS!="" || Shared.ua.verAndroid!="") && winW>getBreakpointSize() ){
                 _sidebarWrapper.css({position:"absolute", top:0, height:_contentH});
            }else if( Shared.ua.isMobile && ( Shared.ua.veriOS!="" || Shared.ua.verAndroid!="") && winW<getBreakpointSize() ){
                 
                  if(winW<768){
                      _sidebarWrapper.css({position:"absolute", top:45, height:$("#page-content-wrapper").height() });
                  }else{
                  ...
                  //$("#sidebar-wrapper")
                      ...
                      //alert(winW+":::"+getBreakpointSize())
                       
                       if(Shared.ua.isIElt9){
                            _sidebarWrapper.css({position:"absolute", top:0, height:898});
                       }else{
                            _sidebarWrapper.css({position:"absolute", top:0, height:_contentH});
                       }
                  }
    ...
    function mobileMenuViewChk(chk) {
        if (chk) {
            ...
            //모바일일 경우 1depth 클릭후 메뉴를 접었다 다시 열었을때 활성화 되어 있던 것을 초기화 시킴 ~
